...
class CheckApacheStatus < Sensu::Plugin::Check::CLI
    option :busy,
        long: '--busy BUSYWORKERS',
        short: '-b Busy Workers',
        description: 'Busy workers threshold',
        proc: proc(&:to_i),
        default: 150,
        required: false
    ...
    option :uptime,
        long: '--uptime UPTIME',
        short: '-u UPTIME',
        description: 'UPTIME threshold in seconds',
        proc: proc(&:to_i),
        default: 60,
        required: false
    ...
    option :secureport,
        long: '--secureport PORT',
        description: 'SSL Port to check mod_status output',
        default: '443'
    ...
    def run
        ...
        if stats['cpuload'].to_f > config[:load] && !stats['cpuload'].nil?
            ...
            if !stats['cpuload'].nil? or !stats['uptime'].nil? or !stats['busy_workers'].nil?
                ok "CPULoad #{stats['cpuload']} - Uptime: #{stats['uptime']} - Busy workers: #{stats['busy_workers']} "
            end
        end
        critical "Apache failure!"
    end
end
